...
static void extract_enum_members(CBMExtractCtx *ctx, TSNode node, const char *class_qn) {
    ...
    for (uint32_t mi = 0; mi < mc; mi++) {
        ...
        /* Swift/Kotlin: one `enum_entry` may declare several comma-separated
         * case names (`case a, b, c`). Each case identifier is a DIRECT
         ...
         * names. Emit one EnumCase per name — cases were previously dropped
         * entirely because `enum_entry` was not a recognized member kind. */
        if ((ctx->language == CBM_LANG_SWIFT || ctx->language == CBM_LANG_KOTLIN) &&
            strcmp(ts_node_type(member), "enum_entry") == 0) {
