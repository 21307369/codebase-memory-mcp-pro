...
// Re-parse embedded script blocks (spec->embedded_defs) and walk them for
// DEFINITIONS, not imports. Mirrors parse_embedded_imports but runs
// cbm_extract_definitions_body on each inner AST. Because the inner tree is
...
void cbm_extract_embedded_defs(CBMExtractCtx *ctx) {
    const CBMLangSpec *spec = cbm_lang_spec(ctx->language);
    if (!spec || !spec->embedded_defs) {
        return;
    }
    for (const CBMEmbeddedLangSpec *e = spec->embedded_defs; e->script_node_type != NULL; e++) {
        const TSLanguage *embedded_lang = cbm_ts_language(e->embedded_language);
        if (!embedded_lang) {
            continue; /* embedded grammar not linked in — silently skip */
        }
