...
/* Swift maps actor→Actor; class stays Class via the tree-sitter `declaration_kind` field. */
...
/* Swift extension: emitting a (Class) type def for it would clobber the real type's idiomatic
...
TEST(swift_enum_cases_extracted_as_enumcase) {
    CBMFileResult *r = extract("enum Direction {\n"
                               ...
                               "    case east, west\n"
                               "}\n",
                               CBM_LANG_SWIFT, "Direction", "Direction.swift");
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
...
TEST(extract_ts_this_super_receiver_not_flagged) {
    ...
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);
    /* this/super receivers should NOT be flagged as is_method */
    int this_call = 0;
    int super_call = 0;
    int new_call = 0;
    for (int i = 0; i < r->calls.count; i++) {
        const char *cn = r->calls.items[i].callee_name;
        if (strcmp(cn, "this.helper") == 0) {
            this_call++;
            ASSERT_FALSE(r->calls.items[i].is_method);
        } else if (strcmp(cn, "super.render") == 0) {
            super_call++;
            ASSERT_FALSE(r->calls.items[i].is_method);
    ...
    ASSERT_EQ(this_call, 1);
    ASSERT_EQ(super_call, 1);
    ASSERT_EQ(new_call, 1);
    cbm_free_result(r);
    PASS();
}
...
/* TODO: upstream API not in fork — Group H3 (ObjectScript UDL/routine) and Group H4
 * (IRIS Export XML transcoder) tests removed: CBM_LANG_OBJECTSCRIPT_UDL/ROUTINE and
 * cbm_iris_export_to_udl() do not exist in the fork. */
