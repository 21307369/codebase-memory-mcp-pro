...
/* Regression: a same-file `extension` shares the extended type's FQN, so
 * emitting a (Class) type def for it would clobber the real type's idiomatic
 * label via the last-write-wins store upsert. The extension must NOT emit a
 * type def (so the struct stays Struct), while its members are extracted. */
...
/* Swift enum cases are extracted as distinct "EnumCase" nodes (previously
 * dropped entirely — enum_entry was not a recognized member kind). A
 * multi-name line (`case east, west`) splits into one EnumCase per case
 * identifier. */
TEST(swift_enum_cases_extracted_as_enumcase) {
    ...
    ASSERT_NOT_NULL(r);
    /* north + east + west = 3 distinct EnumCase nodes (multi-name split). */
