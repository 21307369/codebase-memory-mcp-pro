...
#include "../src/foundation/constants.h" /* CBM_SZ_* */
...
TEST(tool_search_graph_includes_node_properties) {
    ...
    char *resp = cbm_mcp_server_handle(
        srv, "{\"jsonrpc\":\"2.0\",\"id\":42,\"method\":\"tools/call\","
             ...
             "\"fields\":[\"signature\",\"return_type\",\"is_exported\"],\"limit\":5}}}");
    ASSERT_NOT_NULL(resp);
...
TEST(search_code_full_preserves_utf8_source) {
    ...
    ASSERT_NOT_NULL(doc);
    yyjson_val *rows = yyjson_obj_get(yyjson_doc_get_root(doc), "results");
    ASSERT_NOT_NULL(rows);
    ASSERT_TRUE(yyjson_arr_size(rows) > 0);
    yyjson_val *row = yyjson_arr_get(rows, 0);
    yyjson_val *source_val = yyjson_obj_get(row, "source");
...
TEST(search_code_raw_match_preserves_utf8_content) {
    ...
    yyjson_doc *doc = yyjson_read(inner, strlen(inner), 0);
    ASSERT_NOT_NULL(doc);
    yyjson_val *raw_rows = yyjson_obj_get(yyjson_doc_get_root(doc), "raw_matches");
    ASSERT_NOT_NULL(raw_rows);
    ASSERT_TRUE(yyjson_arr_size(raw_rows) > 0);
    yyjson_val *raw_row = yyjson_arr_get(raw_rows, 0);
    ASSERT_STR_EQ(yyjson_get_str(yyjson_obj_get(raw_row, "content")), "raw-Русский content");
    yyjson_doc_free(doc);
...
TEST(search_code_context_preserves_utf8_context) {
    ...
    yyjson_val *rows = yyjson_obj_get(root, "results");
    ASSERT_NOT_NULL(rows);
    ASSERT_TRUE(yyjson_arr_size(rows) > 0);

    /* v2 format: results carry context/context_start as direct keys. */
    yyjson_val *row = yyjson_arr_get(rows, 0);
    yyjson_val *context_obj = yyjson_obj_get(row, "context");
    ASSERT_NOT_NULL(context_obj);
    ASSERT_STR_EQ(yyjson_get_str(context_obj), context_source);
    ASSERT_EQ(yyjson_get_int(yyjson_obj_get(row, "context_start")), 1);
    yyjson_doc_free(doc);
...
TEST(search_code_invalid_utf8_still_returns_valid_json) {
    ...
    ASSERT_NOT_NULL(rows);
    ASSERT_TRUE(yyjson_arr_size(rows) > 0);
    yyjson_val *source_obj = yyjson_obj_get(yyjson_arr_get(rows, 0), "source");
    ASSERT_NOT_NULL(source_obj);
    const char *safe_source = yyjson_get_str(source_obj);
