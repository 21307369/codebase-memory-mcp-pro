...
/* A non-aggregate function (type(r)) mixed with an aggregate (count(*)) must
 * become a GROUPING dimension — one row per distinct edge type — not collapse
 * every row into a single group. setup_cypher_store has 3 CALLS + 1 DEFINES
 * edge, so the correct result is 2 rows (CALLS=3, DEFINES=1); the historical
 * bug produced a single collapsed row. */
...
SUITE(cypher) {
    ...
    /* Non-aggregate function grouping (type(r)) */
